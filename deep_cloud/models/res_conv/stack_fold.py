...
def combined_term_v1(sparse_neighbors,
                     x_in,
                     x_out,
                     features_in,
                     kernel,
    ...
    = \\sum_{k} xout_{ki} \\sum_q theta_{kqp} \\sum_j n_{ij} f_{qj} * xin_{kj}
    """
    D, F_in, F_out = _check_arg_shapes(sparse_neighbors, x_in, x_out,
                                       features_in, kernel)
    del D, F_in
    ...
    return tf.foldl(accumulate, args, init, parallel_iterations=20)
...
def _get_polynomial_stacks(x_in, x_out, kernels):
    kernel_stack = []
    x_in_stack = []
    x_out_stack = []
    binom_coeffs = []
...
#     out = [impl(
#         sparse_neighbors, x_in, x_out, features_in, bias, kernel_stack[0])]
#     for xi, xo, k in zip(x_in_stack[1:], x_out_stack[1:], kernel_stack[1:])
...
def stacked_conv(sparse_neighbors,
    ...
    x_in_stack, x_out_stack, kernel_stack, = _get_polynomial_stacks(
        x_in, x_out, kernels)
    # insert bias/zeroth order at start
    x_in_stack.insert(0, tf.ones((1, N_in), dtype=dtype))
    x_out_stack.insert(0, tf.ones((1, N_out), dtype=dtype))
    kernel_stack.insert(0, tf.expand_dims(bias, axis=0))
    ...
    return combined_term(sparse_neighbors,
                         tf.concat(x_in_stack, axis=0),
                         tf.concat(x_out_stack, axis=0),
                         features_in,
                         tf.concat(kernel_stack, axis=0),
                         impl=impl)
