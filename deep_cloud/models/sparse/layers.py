...
# @gin.configurable
# class GlobalCloudConvolution(layers.Layer):
...
#                  bias_constraint=None,
#                  **kwargs):
#         super(GlobalCloudConvolution, self).__init__(
#             activity_regularizer=regularizers.get(activity_regularizer),
#             **kwargs)
#         self.filters = int(filters)
#         self.activation = activations.get(activation)
...
#         self.supports_masking = True
#         self.input_spec = [
#             InputSpec(ndim=2),  # [N, F_in] node features
#             InputSpec(ndim=2),  # [S, E] edge features
#             InputSpec(shape=(None,), dtype=tf.int64),  # [N] row_lengths
#         ]
...
#         if self.built:
#             return
#         ns, ef, rl = input_shape[:2]
#         num_node_features = ns[1]
#         num_edge_features = ef[0]
#         if rl[0] != ns[0]:
#             raise ValueError(
#                 'node_features.shape[0] must be the same as row_lengths.shape[0], '
#                 'got {} and {}'.format(ns[0], rl[0]))
#         self.kernel = self.add_weight(
#             'kernel',
#             shape=[num_edge_features, num_node_features, self.filters],
#             initializer=self.kernel_initializer,
...
#         self.input_spec[0] = InputSpec(shape=(None, num_node_features))
#         self.input_spec[1] = InputSpec(shape=(num_edge_features, None))
#         super(GlobalCloudConvolution, self).build(input_shape)

#     def call(self, inputs):
#         node_features, edge_features, row_lengths = (
#             tf.convert_to_tensor(i) for i in inputs)
#         outputs = ops.global_conv(node_features, self.kernel, row_lengths,
#                                   edge_features)
#         if self.use_bias:
#             outputs = tf.nn.bias_add(outputs, self.bias)
#         if self.activation is not None:
