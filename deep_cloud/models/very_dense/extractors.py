...
#         if self.is_ragged:
#             self.row_splits = row_splits_or_k
#         else:
...
#                 local_features = layer_utils.concat(
#                     [local_features, edge_features], axis=-1)
#             else:
...
#                 local_features = tf.keras.layers.Lambda(
#                     tf.math.add_n)([local_features, global_features])
#             else:
#                 local_features = layer_utils.reshape_leading_dim(
#                     local_features, (-1, self.k))
#                 global_features = tf.keras.layers.Lambda(
#                     tf.expand_dims, arguments=dict(axis=-2))(global_features)
#                 local_features = tf.keras.layers.Lambda(
#                     tf.math.add_n)([local_features, global_features])
...
class KPartiteFeatureExtractor(object):
    ...
    def __init__(
        ...
        # self.global_extractors = tuple(global_extractors)
    ...
    def __call__(
        ...
        # # add global features
        # for i, glob in enumerate(self.global_extractors):
        #     if glob is not None:
        #         all_out_node_features[i][K] = glob(
        #             None if global_features is None else global_features[i],
        #             node_features[i], None if global_edge_features is None else
...
def get_base_extractor(
    ...
    return KPartiteFeatureExtractor(local_extractors,
                                    # global_extractors,
                                   )
